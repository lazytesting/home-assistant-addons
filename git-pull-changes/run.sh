...
if [ -n DEPLOYMENT_KEY ]; then
    ...
    if [ $? -eq 0 ]; then
        echo "[Info] Valid SSH connection for ${GIT_URL_PARTS[0]}"
    else
        echo "[Info] No valid SSH connection for ${GIT_URL_PARTS[0]}"
        add-ssh-key
    fi
fi
}
...
function git-synchronize {
    ...
    if [ $? -eq 0 ]; then
        echo "[Info] Git repository exists, start pulling"
        OLD_COMMIT=$(git rev-parse HEAD)
        git pull || { echo "[Error] Git pull failed"; exit 1; }
    else
        echo "[Info] Git repostory doesn't exist"
        git-clone
    fi
}
